# byot.py: Token Migration to Skyflow using INSERT BYOT
...
def csvFileProcessor(script_directory):
    fullFile, directory = getFile('BYOT CSV', script_directory)
...
def main():
    ...
    # get file path to current script
    script_path = Path(__file__).resolve()
    script_directory = script_path.parent

    print(f"BYOT Loader for Skyflow now running...\n")
    header_row, full_file  = csvFileProcessor(script_directory)   # Get csv file headers and rows

    credsFile, directory = getFile('Skyflow Credentials', script_directory)
